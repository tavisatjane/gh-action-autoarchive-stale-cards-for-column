...
async function fetchCardInfo(
  ...
  try {
    ...
    while (nextPage) {
      let projectCards = await fetchCards(
        repoOwner,
        repo,
        projectName,
        currentCursor,
      ...
      projectCards =
        projectCards.repository.projects.edges[0].node.columns.edges.find(
          (edge) =>
            edge.node.name.toLowerCase() === columnToArchive.toLowerCase()
        ).node.cards;
      projectCardIdsWithDate.push(
        ...projectCards.edges.flatMap((card) => card.node).map(dateifyCard)
      );
...
const run = async () => {
  try {
    ...
    console.log(
      `Archiving all cards that have been untouched for ${daysOld} days from column ${columnToArchive}!`
    );

    console.log(`The event payload: ${payload}`);

    const projectCardIdsWithDate = await fetchCardInfo(
      repoOwner,
      repo,
      projectName,
      accessToken,
